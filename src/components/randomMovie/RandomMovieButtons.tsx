...
import Modal from '../trailer/TrailerModal';
import styles from './randomMovie.module.css';
import ReactPlayer from 'react-player';
import { Link } from 'react-router-dom';

interface RandomMovieButtonsProps {
    onRefresh: () => void;
    data: { trailerUrl: string } | null;
    buttons: boolean;
}

const RandomMovieButtons: React.FC<RandomMovieButtonsProps> = ({ onRefresh, data, buttons }) => {
    ...
    return (
        <div className={styles.randomMovieButtons}>
            <button
            ...
            {buttons && (
                <Link to={'/about-movie'}>
                    <button className={styles.aboutMovie}>
                        О фильме
                    </button>
                </Link>
            )}
            ...
            <button
                onClick={handleClick}
                className={`${styles.like} ${styles.likeRandomMovie}`}
            >
                {isClicked ? (
            ...
            {buttons && (
                <button className={`${styles.like} ${styles.newRandomMovie}`} onClick={onRefresh}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                <ReactPlayer url={data?.trailerUrl} controls={true} playing={true} />
            </Modal>
        </div>
    );
};

export default RandomMovieButtons;